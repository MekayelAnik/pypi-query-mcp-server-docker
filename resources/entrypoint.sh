...
readonly DEFAULT_PORT=8055
readonly DEFAULT_INTERNAL_PORT=38056
...
readonly HAPROXY_SERVER_NAME="pqms"
...
start_mcp_server() {
    ...
    PQMS_MAX_MEM_MB="${PQMS_MAX_MEM_MB:-0}"

    local pqms_argv=(pypi-query-mcp-server)

    # mcp-proxy receives the stdio command as positional args after `--`,
    # so prlimit can prefix the argv list directly.
    if [[ "${PQMS_MAX_MEM_MB}" =~ ^[1-9][0-9]*$ ]] && command -v prlimit >/dev/null 2>&1; then
        local mem_bytes=$((PQMS_MAX_MEM_MB * 1024 * 1024))
        pqms_argv=(prlimit "--as=${mem_bytes}" -- "${pqms_argv[@]}")
        echo "PyPI Query MCP child memory cap: ${PQMS_MAX_MEM_MB} MiB (prlimit --as)"
    fi
